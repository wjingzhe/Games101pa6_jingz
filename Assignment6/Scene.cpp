...
Vector3f Scene::castRay(const Ray &ray, int curDepthCount) const
{
    ...
    Material *pMaterial = intersection.pMaterial;
...
//        Vector3f tmp = hitPoint;
        switch (pMaterial->getType()) {
            case REFLECTION_AND_REFRACTION:
            {
                ...
                Vector3f refractionDirection = normalize(refract(ray.direction, N, pMaterial->ior));
                ...
                Vector3f refractionRayOrig = (dotProduct(refractionDirection, N) < 0) ?
                                             hitPoint - N * EPSILON :
                                             hitPoint + N * EPSILON;
                Vector3f reflectionColor = castRay(Ray(reflectionRayOrig, reflectionDirection), curDepthCount + 1);
                Vector3f refractionColor = castRay(Ray(refractionRayOrig, refractionDirection), curDepthCount + 1);
                float kr;
                fresnel(ray.direction, N, pMaterial->ior, kr);
            ...
            case REFLECTION:
            {
                ...
                Vector3f reflectionRayOrig = (dotProduct(reflectionDirection, N) < 0) ?
                                             hitPoint + N * EPSILON :
                                             hitPoint - N * EPSILON;
                hitColor = castRay(Ray(reflectionRayOrig, reflectionDirection),curDepthCount + 1) * kr;
                break;
            }
            default:
            {
                ...
                for (uint32_t i = 0; i < get_lights().size(); ++i)
                {
                    ...
                    else
                    {
                        ...
                        specularColor += powf(std::max(0.f, -dotProduct(reflectionDirection, ray.direction)),
                                              pMaterial->specularExponent) * get_lights()[i]->intensity;
                    }
                }
                hitColor = lightAmt * (hitObject->evalDiffuseColor(st) * pMaterial->Kd + specularColor * pMaterial->Ks);
                break;
            }
        }
    }
